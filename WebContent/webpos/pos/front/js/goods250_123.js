...
$(".xuan_ball").click(function(){
	...
	if(select=="select"){
		$(this).attr("select","");
		$(this).attr("style","background-color: rgb(142, 136, 136);");
	}else{
	...
	var rednum=0;
	$(".xuan_ball").each(function(index,item){
		var select = $(this).attr("select");
		if(select=="select"){
			rednum=rednum+1;
		}
	...
	var nowprice = parseFloat($("#nowprice").val());
	var beishu=0;
	if(rednum==5) beishu=1;
	else if(rednum==6) beishu=6;
	else if(rednum==7) beishu=21;
	else if(rednum==8) beishu=56;
	else if(rednum==9) beishu=126;
	else if(rednum==10) beishu=252;
	else if(rednum==11) beishu=462;
	var x = beishu*nowprice;
	$("#b_pay").html(""+x.toFixed(2));
...
$(".sxlist a.ok").click(function(){
	...
	if(rednum<5){
		new TipBox({type:'error',str:'请至少选择5个号码',hasBtn:true});
		return;
	}
